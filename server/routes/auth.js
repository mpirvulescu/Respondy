...
router.post('/register', async (req, res) => {
   ...
   const exists = db.exec('SELECT * FROM users WHERE email = ?', [
      email,
   ]);
   if (exists.length) {
...
router.post('/login', async (req, res) => {
   ...
   const db = await getDb();
   const result = db.exec('SELECT * FROM users WHERE email = ?', [
      email,
   ]);
   if (!result.length) {
   ...
   const user = {id: row[0], name: row[1], email: row[2], role: row[5]};
...
router.post('/reset-password', async (req, res) => {
   ...
   db.run('DELETE FROM password_resets WHERE user_id = ?', [
      userId,
   ]);
   saveDb();
