...
export const callStore = {
  create(callSid, { systemPrompt, guardEnabled = false, userId = null, greeting = null, phoneNumber = null, goal = null } = {}) {
    const entry = {
      status: 'initiated',
      guardEnabled,
      userId,
      ...
      addTranscript(role, text) {
        ...
        getDb().then(db => {
          ...
          saveDb();
        }).catch(err => console.error('Failed to save transcript:', err.message));
      },
    };
    ...
    getDb().then(db => {
      db.run(
        ...
        [callSid, userId, phoneNumber, goal, 'initiated', guardEnabled ? 1 : 0]
      );
      saveDb();
  ...
  updateStatus(callSid, status) {
    ...
    if (entry) {
      entry.status = status;
      const terminal = ['completed', 'failed', 'canceled', 'no-answer', 'busy'];
    ...
    getDb().then(db => {
      ...
      if (terminal.includes(status)) {
        db.run(
          ...
          [status, new Date().toISOString(), callSid]
        );
      } else {
        db.run('UPDATE calls SET status = ? WHERE call_sid = ?', [status, callSid]);
      }
      saveDb();
    }).catch(err => console.error('Failed to update call status:', err.message));
  },

  // Load a call from DB (for when in-memory entry is gone, e.g. server restart)
  async getFromDb(callSid) {
    const db = await getDb();
    const callResult = db.exec('SELECT * FROM calls WHERE call_sid = ?', [callSid]);
  ...
  async listByUser(userId) {
    ...
    for (const call of calls) {
      ...
      const lines = (tResult[0]?.values || []).map(r =>
        `${r[0]}: ${r[1]}`
      );
