...
export default function AdminDashboard() {
   ...
   const [
      stats,
      setStats,
   ] = useState({totalUsers: 0, totalCalls: 0, callsByUser: []});
   const [
      injections,
      setInjections,
   ] = useState([]);
   ...
   return (
      <>
         ...
         <div className='dashboard'>
                     ...
                     <div className='card__content'>
                        {[
                           ...
                           },
                           {
                              label: 'Failed',
                              color: 'red',
                              count: callCounts.failed,
                           },
                           {
                              label: 'Pending',
                              color: 'yellow',
                              count: callCounts.pending,
                           },
                           {
                              label: 'In Progress',
                              color: 'blue',
                              count: callCounts.inProgress,
                           },
                        ].map((row) => (
                           <div key={row.label} className='breakdown-row'>
                              <div className='breakdown-row__left'>
                                 <span
                                    className={`summary-row__dot summary-row__dot--${row.color}`}
                                 ></span>
                                 {row.label}
                              </div>
                              <div className='breakdown-row__right'>
                                 <div className='breakdown-row__bar'>
                                    <div
                                       ...
                                       style={{
                                          width: `${callCounts.total > 0 ? (row.count / callCounts.total) * 100 : 0}%`,
                                       }}
                                    />
                                 </div>
                        ...
                        <div className='alert alert--danger'>
                           <div className='alert__text'>
                              <div className='alert__title'>
                                 {injections.length} Injection Attempt
                                 {injections.length !== 1 ? 's' : ''}
                              </div>
                              <div className='alert__description'>
                                 All attempts have been logged and blocked
                              </div>
                           </div>
                        </div>
            ...
            <div
               className={`tab-panel${activeTab === 'users' ? ' tab-panel--active' : ''}`}
            >
               <div className='card'>
                                    ...
                                    <tr key={u.id}>
                                       <td>
                                          <div className='table__user-cell'>
                                             <span className='table__user-name'>
                                                {u.name}
                                             </span>
                                             <span className='table__user-email'>
                                                {u.email}
                                             </span>
                                          </div>
                                       </td>
                                          ...
                                          <div className='mini-bar'>
                                             <div className='mini-bar__track'>
                                                <div
                                                   ...
                                                   style={{
                                                      width: `${Math.min((u.api_calls_used / 20) * 100, 100)}%`,
                                                   }}
                                                />
                                             </div>
                                             <span className='mini-bar__text'>
                                                {u.api_calls_used}
                                             </span>
                                          </div>
                                       </td>
            ...
            <div
               className={`tab-panel${activeTab === 'calls' ? ' tab-panel--active' : ''}`}
            >
               <div className='card'>
                  <div className='card__content--flush'>
                     {allCalls.length === 0 ? (
                        <div className='empty-state'>
                           ...
                           <div className='empty-state__text'>
                              Calls will appear here once users start making
                              them
                           </div>
                        </div>
                     ) : (
            ...
            <div
               className={`tab-panel${activeTab === 'settings' ? ' tab-panel--active' : ''}`}
            >
               <div className='card'>
                  <div className='card__header'>
                  ...
                  <div className='card__content'>
                     <div className='form-group'>
                        ...
                        <div className='form-helper'>
                           The user's call goal is automatically appended to
                           this prompt for each call.
                        </div>
                     </div>
                     <div
                        style={{
            ...
            >
               {injections.length === 0 ? (
                  ...
                  <div>
                     ...
                     >
                        {injections.length} prompt injection attempt
                        {injections.length !== 1 ? 's' : ''} detected
                     </div>
                     {injections.map((log) => (
                        <div key={log.id} className='injection-card'>
                           ...
                           <div className='injection-card__text'>
                              {log.input_text}
                           </div>
